...
out_dir <- file.path(repo_root, "r", "output", "featureSelection", "l1regularization")
...
# For run_selectionsmodel.R: nonzero at lambda.min (same default as FEATURE_LAMBDA in l1selectionsmodel.r)
fn_min <- rownames(cm_min)[-1L]
v_min <- as.numeric(cm_min[-1L, 1L])
sel_lmin <- fn_min[abs(v_min) > 1e-8]
utils::write.csv(
  data.frame(feature = sel_lmin, stringsAsFactors = FALSE),
...
cat(" ", normalizePath(file.path(out_dir, "selected_features.csv")), "\n")
